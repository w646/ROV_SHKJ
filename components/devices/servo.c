...
#define HEAD_SOF 0x124C //帧头


uint8_t servo_tx_buffer[12];
...
/**
  ...
  * @retval         计算完的校验和
  */
static uint8_t get_check_sum(uint8_t *pch_message,uint32_t dw_length)
{
   uint8_t check_sum = 0;
    if (pch_message == NULL)
    {
...
* @retval			返回要发送的数据大小
*/
static uint16_t send_servo_pack(uint8_t cmd_type, uint8_t *p_data, uint8_t len)
{
    ...
    get_check_sum(servo_tx_buffer, send_len);
    uart7_tx_dma_enable(servo_tx_buffer, send_len);
    return send_len;
}
 ...
 * @retval none
*/
void set_servo_angle(uint8_t servo_id, int16_t angle_set, uint16_t time_ms)
{
    uint8_t data[7];
    data[0] = servo_id;  
 ...
 * @retval none
*/
void get_servo_angle(uint8_t servo_id)
{
    uint8_t data[1];
    data[0] = servo_id;  
 ...
 * @retval none
*/
void modify_servo_id(uint8_t servo_id, uint8_t new_id)
{
    uint8_t data[3];
    data[0] = servo_id;  
    data[1] = 0x22; 
