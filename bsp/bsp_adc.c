...
static uint16_t adcx_get_chx_value(ADC_HandleTypeDef *ADCx, uint32_t ch)
{
    ...
    sConfig.Rank = 1;
		...
		sConfig.Offset=0;
    sConfig.SamplingTime = ADC_SAMPLETIME_810CYCLES_5;//ADC_SAMPLETIME_2CYCLES_5;
...
fp32 sampling_cpu_temprate(void)
{
    ...
    TS_CAL1 = *(__IO uint16_t *)(0x1FF1E820);
		...
		temperate = ((110.0f - 30.0f) / (TS_CAL2 - TS_CAL1)) * (adcx - TS_CAL1) + 30.0f;
...
//	  voltage_vdda_real = 3.3*VREFINT_CAL/(2.0*adcx_get_chx_value(&hadc3, ADC_CHANNEL_VREFINT));
		//分压电路所以实际电压值应该×2
		adcx = adcx_get_chx_value(&hadc3, ADC_CHANNEL_2);
    voltage =  (fp32)adcx /65535 * 3.3f;
    return voltage;
}
